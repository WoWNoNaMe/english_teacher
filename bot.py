...
def generate_daily_words(previous_words: list[str]) -> str:
    """5 új szakmai angol szó/kifejezés generálása Credit Risk Model Validator témában."""
    ...
    response = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""Te egy szakmai angol oktató vagy. A tanuló Junior Model Validator pozícióban dolgozik – pénzügyi modellek validálásával, kvantitatív kockázatelemzéssel, stresszteszteléssel, model governance-szel foglalkozik.
...
def generate_review_question(word_entry: str, all_today_words: list[str]) -> str:
    ...
    response = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""Egy Junior Model Validator angolt tanul. Ez az egyik mai szava:
{word_entry}
...
def check_answer(question: str, word_entry: str, user_answer: str) -> str:
    ...
    response = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""Kérdés volt: {question}
